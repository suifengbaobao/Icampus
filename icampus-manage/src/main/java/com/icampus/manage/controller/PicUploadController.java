...
import com.icampus.common.bean.PicUploadResult;
import com.icampus.manage.service.PropertyService;
...
public class PicUploadController {
	...
	public String upload(@RequestParam("uploadFile") MultipartFile uploadFile , HttpServletResponse response) throws Exception {
		...
		isLegal = false;
		try {
			BufferedImage image1 = ImageIO.read(newFile);
			if (uploadFile != null) {
				fileUploadResult.setWidth(image1.getWidth() + "");
				fileUploadResult.setHeight(image1.getHeight() + "");
				isLegal = true;
			}
		} catch (IOException e) {
		}
